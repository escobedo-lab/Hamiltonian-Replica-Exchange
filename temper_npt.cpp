...
//    error->universe_all(FLERR,"Tempering temperature and pressure fix is not supported");
  ...
  int id = modify->find_compute("for_temper_npt"); //MODIFIED FOR HREX SIMULATIONS
  if (id < 0) error->all(FLERR,"Tempering could not find compute");
  ...
  if (narg == 8) {
    ...
    modify->fix[whichfix]->reset_target(new_temp);//MODIFIED FOR HREX SIMULATIONS
  }
...
//ACCEPTANCE FOR REAL UNITS NPT HREX SIMULATIONS at T=600 K, P=1 atm, and spring constant k= 15.0
        delr= (1.0/(boltz*600.0))*(15.0/2.0)*(((pe_partner-set_temp[my_set_temp])*(pe_partner-set_temp[my_set_temp]))-((pe-set_temp[my_set_temp])*(pe-set_temp[my_set_temp]))+((pe-set_temp[partner_set_temp])*(pe-set_temp[partner_set_temp]))-((pe_partner-set_temp[partner_set_temp])*(pe_partner-set_temp[partner_set_temp])));

//ACCEPTANCE FOR LJ TEST SYSTEM

//	delr= (1.0/(boltz*1.0))*(600.0/2.0)*(((pe_partner-set_temp[my_set_temp])*(pe_partner-set_temp[my_set_temp]))-((pe-set_temp[my_set_temp])*(pe-set_temp[my_set_temp]))+((pe-set_temp[partner_set_temp])*(pe-set_temp[partner_set_temp]))-((pe_partner-set_temp[partner_set_temp])*(pe_partner-set_temp[partner_set_temp])));
...
#ifdef TEMPER_DEBUG
      if (me_universe < partner)
        fprintf(universe->uscreen,"SWAP %d & %d: yes = %d,Ts = %d %d, setTs = %g %g, Bz = %g %g, PEs = %g %g,boltz=%g\n",
               me_universe,partner,swap,my_set_temp,partner_set_temp,set_temp[my_set_temp],
               set_temp[partner_set_temp],boltz_factor,exp(boltz_factor), pe, pe_partner,boltz);
#endif
...
//    if (swap) scale_velocities(partner_set_temp,my_set_temp);
    ...
    if (swap) {
      ...
      modify->fix[whichfix]->reset_target_BP(new_temp); //MODIFIED FOR HREX SIMULATIONS
    }
